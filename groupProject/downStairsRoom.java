...
public class downStairsRoom extends Screen{ 
	...
	public void update(){ 
		...
		if(p1.x>=0 && p1.x<= 100){ 
			if(p1.y<=85){  
			p1.y=95;
				MyGame.activeScreen=MyGame.g3; 
			} 
			if(p1.y>=580){ 
				p1.y=570;
				MyGame.activeScreen=MyGame.g7;
			}			
	...
	public void draw(Graphics pen){ 
			...
			if(printLock){ 
				lockPrint.draw(pen); 
				Sign newSign=new Sign(0,100,200,50,Color.WHITE,"You have defeated "+MyGame.bossCount+"/4",Color.BLACK,15); 
				newSign.draw(pen);
			}
